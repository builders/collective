...
<xsl:template match="data" mode="js">
	...
	<script type="text/javascript" src="{$workspace}/assets/intranet/js/syntax-xml.js"></script>
...
<xsl:template match="forum-comments">
	<section class="main">
		<article class="content">
			<h1 class="heading"><xsl:value-of select="/data/forum-discussions/entry/topic"/></h1>
...
<xsl:template match="forum-comments/entry">
	...
	<div id="position-{$comment-position}" class="comment">
		<div class="meta">
			<ul class="avatar">
				<li class="gicon">
					<img src="http://www.gravatar.com/avatar/{/data/forum-members/entry[@id = current()/created-by/item/@id]/email-address/@hash}?s=50" width="50" height="50"/>
				</li>
			...
			<ul>
				...
				<xsl:choose>
					<xsl:when test="created-by/item/@id = 2101 or created-by/item/@id = 2102 or created-by/item/@id = 2103 or created-by/item/@id = 5061">
						<li class="dev icon">dev team</li>
					</xsl:when>
					<xsl:when test="created-by/item/@id = /data/forum-moderators/entry/@id">
...
<xsl:template name="notifications">
	<h3>Notifications</h3>
	<form method="post" action="{$current-url}" id="notification-form">
		<input name="fields[discussion-topic]" type="hidden" value="{$discussion-id}" />
		<xsl:choose>
			<xsl:when test="/data/forum-discussion-subscriber/entry">
				<xsl:choose>
					<xsl:when test="/data/forum-discussion-subscriber/entry[subscribe = 'Yes']">
							<p>You are subscribed to email notifications for this discussion.</p>
<xsl:for-each select="/data/forum-discussion-subscriber/entry[subscribe = 'Yes']">
							<input name="fields[subscriber]" type="hidden" value="{$member/@id}" />
							<input name="fields[subscribe]" type="hidden" value="no" />
							<input name="id" type="hidden" value="{@id}" />
							<input name="action[forum-notifications]" type="submit" value="Unsubscribe" />
						</xsl:for-each>
					</xsl:when>
					<xsl:otherwise>
						<xsl:for-each select="/data/forum-discussion-subscriber/entry">
							<p>Subscribe to email notifications for this discussion:</p>
							<input name="fields[subscriber]" type="hidden" value="{$member/@id}" />
							<input name="fields[subscribe]" type="hidden" value="yes" />
							<input name="id" type="hidden" value="{@id}" />
							<input name="action[forum-notifications]" type="submit" value="Notify Me" />
						</xsl:for-each>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
...
<input name="fields[subscriber]" type="hidden" value="{$member/@id}" />
	...
	<xsl:if test="/data/forum-subscription-members/entry">
		<h4>Subscribers</h4>
		<ul class="menu list-menu">
			<xsl:apply-templates select="/data/forum-subscription-members/entry" />
		</ul>
	</xsl:if>
</xsl:template>

<xsl:template match="/data/forum-subscription-members/entry">
	<li><a href="{$root}/people/{name/@handle}/" title="{username}"><xsl:value-of select="name" /></a></li>
</xsl:template>
