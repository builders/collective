...
<xsl:param name="filter" select="$type" />
<xsl:param name="filter-title" select="/data/project-types/entry[title/@handle = $type]/title" />
				...
				<li>Project Type
					<ul>
						<xsl:for-each select="project-types/entry">
							<li>
								<xsl:if test="$type = title/@handle"><xsl:attribute name="class">active</xsl:attribute></xsl:if>
								<a href="{$root}/results/project/{title/@handle}/">
									<xsl:value-of select="title"/>
									<xsl:if test="$type = title/@handle"> &gt;</xsl:if>
								</a>
							</li>
						</xsl:for-each>
					</ul>
...
<xsl:template name="work-rows">
	...
	<xsl:param name="total" select="count(work-by-type/entry)"/>
	<div class="row">
...
<xsl:template name="work-thumbs">
	...
	<xsl:for-each select="work-by-type/entry[$count]">
