...
<xsl:template match="data" mode="activate">
	...
	<p>Thanks for being a part of the team<xsl:if test="$member">, <xsl:value-of select="$member/username"/></xsl:if>!</p>
	...
	<form method="post" action="{$current-url}" class="members-form">
		<fieldset>
			<p>
				...
				<input id="email" name="fields[email-address]" type="text" value="{$email}" />
			</p>
			<p>
...
<xsl:template match="data" mode="resend">
	...
	<xsl:for-each select="$resend-event[@result = 'success']/activation-code">
		<p class="success">The code was emailed to you at <xsl:value-of select="$member/email-address" /></p>
	</xsl:for-each>
	<form method="post" action="{$current-url}" class="members-form">
		<fieldset>
			<p>
