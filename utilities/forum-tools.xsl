...
<xsl:template name="tagify">
	...
	<xsl:variable name="comment-meta">
		<xsl:value-of select="concat('Posted by ', member, ' on ')"/>
		<xsl:call-template name="format-date">
	...
	<xsl:choose>
		...
		<xsl:when test="$search-results = 1">
			<a href="{$root}/forum/discussions/{$forum-id}/" title="{$comment-meta}">
